# run this torchrun --standalone --nproc_per_node=8 gpt2.py
import os
import math
import time
import inspect
...
#         att = att.masked_fill(self.bias[:,:,:T,:T] == 0, flat('-inf'))
...
class DataLoaderLite:
    ...
    def next_batch(self):
        ...
        # if loading the next batch out of bound, reset 
...
# Arbitary batch size simulation with gradient accumulation
...
def get_lr(it):
    ...
    # 2) if it > lr_decays_iters, return min learning rate
    if it > max_steps:
        return min_lr
...
for step in range(max_steps):
    ...
    for micro_step in range(grad_accum_steps):
        ...
        loss.backward() # This deposit or add the gradient 
    if ddp:
    ...
    if master_process:
        print(f"step {step:4d} | loss: {loss_accum.item():.6f} | lr: {lr:.4e} | norm: {norm:.4f} | dt:{dt:.2f}ms | tok/sec: {token_per_sec}")
